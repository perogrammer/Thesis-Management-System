                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        if (userId > 0)
                        {
                            ...
                            query = "SELECT COUNT(*) FROM ThesisSupervisors WHERE UserID = @UserID;";
                            ...
                            if (studentCount > 0)
                            {
                                query = "SELECT SupervisorID FROM ThesisSupervisors WHERE UserID = @UserID;";
                                command.Parameters.Clear();
                                command.CommandText = query;
                                command.Parameters.AddWithValue("@UserID", userId);

                                Session["SupervisorID"] = (int)command.ExecuteScalar();
                                // Successful login for a student
                                Response.Write("<script>alert('Logged in as Supervisor.');</script>");
